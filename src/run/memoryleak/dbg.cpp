 ...
 * memory block, and two-unsigned char guards before and after the requested memory
 * block.  The requested memory block is filled with a semi-random unsigned char
 ...
 * (possibly different) unsigned char value after deallocation to ensure that the
...
void dump_alloc_heaps(void) {
  if (g_heap || g_vector_heap) {
    ...
    dbgout("=====================");
    dbgout("Total unsigned chars: %u", ttl);
    dbgout("=====================");
  }
}
