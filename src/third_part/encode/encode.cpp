...
static void GBcode_Unicode(unsigned char *pout, unsigned char *pintput) {
  unsigned int i;
  pout[0] = 0;
  pout[1] = 0;
...
static void Unicode_GBcode(unsigned char *pout, unsigned char *pintput) {
...
// ��UTF-8ת����Unicode
static void UTF_8ToUnicode(unsigned char* pOut, unsigned char *pText) {
  pOut[0] = ((pText[0] & 0x0F) << 4) + ((pText[1] >> 2) & 0x0F);
  pOut[1] = ((pText[1] & 0x03) << 6) + (pText[2] & 0x3F);
  return;
}
// Unicode ת����UTF-8
static void UnicodeToUTF_8(unsigned char* pOut, unsigned char* pchar) {
...
bool Gb2312ToUtf8(std::string &outStr,
  ...
  int new_size = RGN_Gb2312_To_UTF8((unsigned char *)(void *)outStr.c_str(),
                                    outStr.size(),
                                    (unsigned char *)(void *)inputStr.c_str(),
                                    inputStr.size());
...
int RGN_Gb2312_To_UTF8(unsigned char *pOut, int pOutLen,
  ...
  while (i < pLen) {
    if (*(pText + i) <= 160) {
      ...
      UnicodeToUTF_8((unsigned char*)(pOut + j), pdata);
      j += 3;
      i += 2;
    }
  }
...
int RGN_UTF8_To_Gb2312(unsigned char *pOut, int pOutLen,
  ...
  while (i < pLen) {
    if (pText[i] < 160) {
      ...
      UTF_8ToUnicode(pdata, (unsigned char*)(pText + i));
